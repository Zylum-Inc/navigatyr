...
pub fn get_cpp_extra_flags(
    device_id: &str,
    config_file: &Path,
) -> Result<String, Error> {
    if !config_file.exists() {
        return Err(Error::msg(format!("Config file {:?} does not exist", config_file)));
    }
    ...
    if device_id != doc["Device"]["id"].as_str().unwrap() {
        return Err(Error::msg(format!("DeviceID in config file {:?} does not match device_id {:?}",
    ...
    println!("num_device_config_elements: {:?}", num_device_config_elements);

    for i in 0..num_device_config_elements {
        println!("i: {}, DeviceConfig[i]: {:?}, DeviceConfig[i][compile_time_prefix]: {:?}",
                 i, doc["DeviceConfig"][i], doc["DeviceConfig"][i]["compile_time_prefix"]);
        cpp_extra_flags.push_str(doc["DeviceConfig"][i]["compile_time_prefix"].as_str().unwrap());
    ...
    println!("num_network_config_elements: {:?}", num_network_config_elements);

    for j in 0..num_network_config_elements {
        cpp_extra_flags.push_str(doc["Networks"][0]["config"][j]["compile_time_prefix"].as_str().unwrap());
...
fn test_get_cpp_extra_flags() {
    let result =
        get_cpp_extra_flags("216fa23d-8fda-4a17-8efa-93d45796dcf3",
                            Path::new("test.yaml"));
...
fn test_get_cpp_extra_flags_device_id_mismatch() {
    let result =
        get_cpp_extra_flags("216fa23d-8fda-4a17-8efa-93d45796dcf4",
                            Path::new("test.yaml"));
